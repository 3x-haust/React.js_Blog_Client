...
interface EditorBlockProps {
  ...
  index: number;
  isSelected?: boolean;
  onToggleSelect?: (id: string, multi: boolean, shift: boolean, index: number) => void;
...
export function EditorBlock({ block, index, isSelected = false, onToggleSelect, onUpdate, onDelete, onMove, onInsertBlock }: EditorBlockProps) {
  ...
  return (
    <div
      ...
      className={`group relative py-4 pl-10 pr-12 border transition-colors rounded-md select-none [&_textarea]:select-text [&_.ql-editor]:select-text ${isSelected ? 'border-primary/60 bg-primary/5' : isOverCurrent ? 'border-primary/40 bg-muted/20' : 'border-transparent hover:border-border/60 hover:bg-muted/20'} ${isDragging ? 'opacity-50' : 'opacity-100'}`}
      onClick={(e) => {
        if (!onToggleSelect) return;
        onToggleSelect(block.id, e.metaKey || e.ctrlKey, e.shiftKey, index);
      }}
    >
      <div
